...
def test_generated_fugue():
    ...
    for i in range(num_positions):
        ...
        voice_dict = {
            ...
            "tenor": Pitch(generator.voices[Voice.TENOR][i]),
    ...
    # ============================================================
    # 3. 声部間隔チェック（上3声: S-A, A-T は1オクターブ以内）
    # ============================================================
    ...
    for i in range(num_positions - 1):
        voices_list = [Voice.SOPRANO, Voice.ALTO, Voice.TENOR, Voice.BASS]
        ...
        for j in range(len(voices_list)):
            for k in range(j + 1, len(voices_list)):
                ...
                if not valid:
                    violations.append(
                        f"  拍{i}→{i+1} {v1.value}-{v2.value}: {msg}"
                    )
    ...
    for voice in Voice:
        for i in range(num_positions - 1):
            ...
            if not valid:
                ...
                augmented_violations += 1

    if augmented_violations == 0:
        print("  ✓ 増音程の旋律的使用なし")
    ...
    for i in range(len(generator.progression) - 1):
        ...
        if not valid:
            violations.append(
                f"  {i}→{i+1}: {curr_degree.name}→{next_degree.name} {msg}"
            )
    ...
    for i in range(num_positions):
        ...
        pitches = [
            ...
            Pitch(generator.voices[Voice.TENOR][i]),
    ...
    if completeness_violations == 0:
        print("  ✓ 全ての和音が完全（第三音省略・重複なし）")
    ...
    success_rate = (
        (total_checks - len(violations)) / total_checks * 100
        if total_checks > 0
        else 0
    )
    print(f"\n成功率: {success_rate:.1f}%")
