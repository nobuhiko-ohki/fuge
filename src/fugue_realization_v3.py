...
class SubjectHarmonicAnalyzer:
    ...
    def analyze(self, subject: Subject) -> List[ChordLabel]:
        ...
        # --- 同一和音の過度な連続を修正 ---
        # ダミーPitchリストを作成（_reduce_repetitionの互換性のため）
...
class FugueRealizationEngine:
    ...
    def __init__(self, fugue_structure: FugueStructure,
        ...
        self.analyzer = SubjectHarmonicAnalyzer(self.key, seed=seed)
    ...
    def realize_exposition(
        ...
        answer_analyzer = SubjectHarmonicAnalyzer(
            self.key.get_dominant_key(), seed=self.seed)
        answer_chord_plan = answer_analyzer.analyze(answer)
