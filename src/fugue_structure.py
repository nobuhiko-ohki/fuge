...
class Key:
    ...
    def __post_init__(self):
        """型安全性チェック: 整数が渡された場合は音名に変換"""
        if isinstance(self.tonic, int):
            self.tonic = PC_TO_NOTE.get(self.tonic, 'C')
    ...
    def get_dominant_key(self) -> 'Key':
        ...
        長調の属調は長調、短調の属調は短調（属短調）。
        例: D minor → A minor, C major → G major
        """
        dom_pc = self.dominant_pc
        return Key(PC_TO_NOTE.get(dom_pc, 'G'), self.mode)
    ...
    def scale_for_chord(self, chord_tones: Set[int]) -> List[int]:
        """コード構成音に整合するスケールを返す。

        優先順位: 主題 > コード > スケール
        コードに導音が含まれるか否かでスケール種別を決定する。

        短調の場合:
          - コードに導音(#7)が含まれる → 和声的短音階（導音を含む）
          - コードに導音が含まれない   → 自然短音階（自然7度）
        長調の場合:
          - 常に長音階を返す

        Args:
            chord_tones: 当該拍のコード構成音（ピッチクラスのSet）
        ...
        if self.mode == "major":
            return rules.get_major_scale(self.tonic_pc)
        # 短調: 導音の有無で判定
        leading_tone = self.leading_tone_pc  # 和声的短音階の第7音
        if leading_tone in chord_tones:
            return rules.get_harmonic_minor_scale(self.tonic_pc)
        else:
            return rules.get_natural_minor_scale(self.tonic_pc)

    @property
    def natural_minor_scale(self) -> List[int]:
        """自然短音階のピッチクラス列（長調の場合は長音階）"""
        rules = HarmonyRules()
        if self.mode == "major":
            return rules.get_major_scale(self.tonic_pc)
        return rules.get_natural_minor_scale(self.tonic_pc)
...
class Subject:
    ...
    notes: List[NoteEvent] — 音高＋音長のリスト
    Pitch リストでも構築可能（全音符が四分音符として扱われる）
    """

    def __init__(self, notes_or_pitches, key: 'Key', name: str = "主題",
                 harmonic_template=None):
        # Pitch リストが渡された場合は四分音符NoteEventに変換（後方互換）
        if notes_or_pitches and isinstance(notes_or_pitches[0], Pitch):
            self.notes: List[NoteEvent] = [NoteEvent(p, 4) for p in notes_or_pitches]
        else:
            self.notes: List[NoteEvent] = list(notes_or_pitches) if notes_or_pitches else []
        self.key = key
        self.name = name
        self.harmonic_template = harmonic_template  # SubjectHarmonicTemplate or None

    @property
    def pitches(self) -> List[Pitch]:
        """後方互換: 音高リスト（各NoteEventの音高を返す）"""
        return [n.pitch for n in self.notes]

    def get_length(self) -> int:
        """主題の長さ（拍数 = 四分音符換算、天井除算）

        サブビート数が4の倍数でない場合は切り上げ。
        例: 66サブビート → 17拍（最終拍は2サブビートのみ使用）
        """
        sb = self.get_length_subbeats()
        return (sb + 3) // 4

    def get_length_subbeats(self) -> int:
        """主題の長さ（サブビート単位: 十六分音符=1）"""
        return sum(n.duration for n in self.notes)

    def transpose(self, semitones: int) -> 'Subject':
        """主題を移調（音長保持）"""
        transposed = [NoteEvent(Pitch(n.pitch.midi + semitones), n.duration)
                      for n in self.notes]
        return Subject(transposed, self.key, f"{self.name}(移調)")

    def diatonic_transpose_to(self, from_key: 'Key', to_key: 'Key') -> 'Subject':
        """主題をダイアトニック移調する（音階度数を保持）

        各音を from_key のスケール内位置（度数）に分解し、
        同じ度数を to_key のスケールに割り当て直す。
        導音など臨時記号（スケール外の音）は to_key でも対応する
        臨時記号に変換する。

        Args:
            from_key: 原調（主題が書かれている調）
            to_key: 目標調

        Returns:
            to_key にダイアトニック移調された新しい Subject
        """
        from_scale = from_key.natural_minor_scale  # 7音
        to_scale = to_key.natural_minor_scale  # 7音
        # 和声的短音階の導音マッピング
        from_leading = from_key.leading_tone_pc
        to_leading = to_key.leading_tone_pc

        transposed = []
        for note in self.notes:
            pc = note.pitch.pitch_class
            octave = note.pitch.midi // 12

            # 導音（臨時記号）の場合
            if pc == from_leading:
                new_pc = to_leading
            else:
                # from_scale 内の最近接度数を見つける
                best_idx = 0
                best_dist = 12
                for i, s in enumerate(from_scale):
                    dist = min(abs(pc - s), 12 - abs(pc - s))
                    if dist < best_dist:
                        best_dist = dist
                        best_idx = i
                new_pc = to_scale[best_idx]

            # 新しい MIDI 値を計算
            new_midi = octave * 12 + new_pc

            # 元の音との相対的な距離を維持するためにオクターブ調整
            # from_key → to_key の基本シフト量
            base_shift = (to_key.tonic_pc - from_key.tonic_pc) % 12
            if base_shift > 6:
                base_shift -= 12
            expected_midi = note.pitch.midi + base_shift
            # 期待値から1オクターブ以上ずれていたら補正
            while new_midi - expected_midi > 6:
                new_midi -= 12
            while expected_midi - new_midi > 6:
                new_midi += 12

            transposed.append(NoteEvent(Pitch(new_midi), note.duration))

        return Subject(transposed, to_key, f"{self.name}(移調→{to_key.tonic}{to_key.mode})")
    ...
    def _tonal_answer(self) -> 'Subject':
        ...
        # 頭部/尾部分割（NoteEvent単位）
        head_notes, tail_notes = self._get_head_tail_notes()
        ...
        for n in head_notes:
            ...
            if degree == 4:
                answer_notes.append(NoteEvent(Pitch(n.pitch.midi + 5), n.duration))
            else:
                answer_notes.append(NoteEvent(Pitch(n.pitch.midi + 7), n.duration))
        ...
        return Subject(answer_notes, dom_key, "応答（調的）")

    def _get_head_tail_notes(self) -> Tuple[List[NoteEvent], List[NoteEvent]]:
        """NoteEvent単位の頭部/尾部分割"""
        dominant_pc = self.key.dominant_pc
        for i, n in enumerate(self.notes):
            if n.pitch.midi % 12 == dominant_pc:
                return self.notes[:i + 1], self.notes[i + 1:]
    ...
    def invert(self) -> 'Subject':
        ...
        axis = self.notes[0].pitch.midi
        inverted = [NoteEvent(Pitch(axis - (n.pitch.midi - axis)), n.duration)
                    for n in self.notes]
        return Subject(inverted, self.key, f"{self.name}(反転)")

    def retrograde(self) -> 'Subject':
        """主題を逆行（音長も逆順）"""
        rev = list(reversed(self.notes))
    ...
    def augmentation(self, factor: int = 2) -> 'Subject':
        """主題を拡大（音価を整数倍）"""
        aug = [NoteEvent(n.pitch, n.duration * factor) for n in self.notes]
        return Subject(aug, self.key, f"{self.name}(拡大×{factor})")

    def diminution(self, factor: int = 2) -> 'Subject':
        """主題を縮小（音価を整数分の1、最小1サブビート）"""
        dim = [NoteEvent(n.pitch, max(1, n.duration // factor))
               for n in self.notes]
        return Subject(dim, self.key, f"{self.name}(縮小÷{factor})")

    # ---- 分析 ----

    def get_head_tail_split(self) -> Tuple[List[Pitch], List[Pitch]]:
        """主題を「頭部」と「尾部」に分割（Prout Ch.III §4）— Pitch単位（後方互換）
        ...
        for i, p in enumerate(pitches):
            if p.midi % 12 == dominant_pc:
                return pitches[:i + 1], pitches[i + 1:]
        return list(pitches), []
...
class Codetta:
    ...
    def __init__(self, notes_or_pitches, name: str = "コデッタ"):
        if notes_or_pitches and isinstance(notes_or_pitches[0], Pitch):
            self.notes: List[NoteEvent] = [NoteEvent(p, 4) for p in notes_or_pitches]
        else:
            self.notes: List[NoteEvent] = list(notes_or_pitches) if notes_or_pitches else []
    ...
    def get_length(self) -> int:
        """拍数"""
        return self.get_length_subbeats() // 4
...
class KeyPath:
    ...
    嬉遊部（および将来の他セクション）において、
    各拍がどの調に属するかを表現する。

    バッハのフーガでは任意の瞬間にひとつの調が支配する。
    この構造はその前提に基づき、拍ごとの調を単純なリストで保持する。

    Attributes:
        start_key: 始点の調
        end_key: 終点の調
        beat_keys: 拍ごとの調のリスト（長さ=拍数）
    """
    start_key: 'Key'
    end_key: 'Key'
    beat_keys: List['Key'] = field(default_factory=list)

    @property
    def num_beats(self) -> int:
        return len(self.beat_keys)

    def key_at(self, beat: int) -> 'Key':
        """指定拍の調を返す"""
        if not self.beat_keys:
            return self.start_key
        beat = max(0, min(beat, len(self.beat_keys) - 1))
        return self.beat_keys[beat]

    def modulation_points(self) -> List[int]:
        """調が変わる拍のインデックスを返す"""
        points = []
        for i in range(1, len(self.beat_keys)):
            if self.beat_keys[i] != self.beat_keys[i - 1]:
                points.append(i)
        return points


class KeyPathStrategy:
    """調性経路の生成戦略（差し替え可能）

    ルールベースまたは学習ベースの実装を同一インターフェースで提供する。
    デフォルト実装は「始点調で開始し、末尾で終点調に切り替える」最小限の戦略。

    ML統合点:
        このクラスを継承し generate() をオーバーライドすることで、
        学習済みモデルによる転調経路を注入できる。
    """

    def generate(self, start_key: 'Key', end_key: 'Key',
                 num_beats: int) -> KeyPath:
        """始点調と終点調からKeyPathを生成する

        Args:
            start_key: 嬉遊部冒頭の調
            end_key: 嬉遊部末尾の調（次のセクションの調）
            num_beats: 嬉遊部の拍数
        ...
        if start_key == end_key or num_beats <= 2:
            # 同一調、または短すぎて転調不要
            return KeyPath(start_key, end_key,
                           [start_key] * num_beats)

        # デフォルト: 最後の2拍で終点調に切り替え
        # （転調点はML等で学習する対象）
        pivot = max(1, num_beats - 2)
        beat_keys = ([start_key] * pivot
...
class Episode:
    ...
    step_interval: int = -1          # 各反復の移調幅（音階度数、負=下行）
    source: str = "主題"             # 素材源
    key: Optional['Key'] = None      # 調性（ダイアトニック移調用、始点調）
    start_key: Optional['Key'] = None  # 嬉遊部の始点調（Noneならkeyを使用）
    end_key: Optional['Key'] = None    # 嬉遊部の終点調（Noneならstart_keyと同じ）

    def _diatonic_transpose(self, pitch: Pitch, degree_offset: int) -> Pitch:
        """音階度数単位でダイアトニック移調する

        Args:
            pitch: 元の音高
            degree_offset: 移調する音階度数（負=下行）

        Returns:
            移調後のPitch
        """
        if self.key is None:
            # keyがない場合は半音移調にフォールバック
            return Pitch(pitch.midi + degree_offset * 2)

        # 反復進行では自然短音階を使用（Baroque実践: 順次進行にC♮、導音C#は使わない）
        scale = self.key.natural_minor_scale  # 短調では自然短音階、長調では長音階
        pc = pitch.pitch_class
        octave = pitch.midi // 12
        ...
        # degree_offsetだけ音階度数を移動
        new_idx = best_idx + degree_offset
        new_octave_offset = new_idx // 7
        new_idx = new_idx % 7
        new_pc = scale[new_idx]

        # 新しいMIDI値を計算
        new_midi = (octave + new_octave_offset) * 12 + new_pc

        # 元の音との距離が妥当か確認（オクターブ跳躍を防止）
        if abs(new_midi - pitch.midi) > 12:
            # オクターブ補正
            if new_midi > pitch.midi:
                new_midi -= 12
            else:
                new_midi += 12

        return Pitch(new_midi)

    def generate_pitches(self) -> List[Pitch]:
        """エピソードの音列を生成（Prout Ch.VII §2-4）

        Prout の反復進行（sequence）:
        - 動機を音階度数単位で移調して繰り返す（ダイアトニック反復進行）
        - 下行反復進行が最も一般的
        - 2度下行（step_interval=-1）または3度下行（step_interval=-2）が典型
        """
        all_pitches = []
        for step in range(self.sequence_steps):
            total_degree_offset = step * self.step_interval
            for p in self.motif_pitches:
                all_pitches.append(self._diatonic_transpose(p, total_degree_offset))
        return all_pitches
...
class FugueStructure:
    ...
    def __init__(self, num_voices: int, main_key: Key, subject: Subject,
        ...
        Args:
            ...
            main_key: 主調
            subject: 主題
            entry_overlap: エントリ間の重複拍数（BWV 1080では1）
        """
        self.num_voices = num_voices
        self.main_key = main_key
        self.subject = subject
        self.entry_overlap = entry_overlap
    ...
    def create_exposition(self,
                          ...
                          overlap: int = 0) -> List[FugueEntry]:
        """提示部を生成（Prout Ch.I §1-3, Gédalge §4.2）
        ...
        Args:
            ...
            overlap: エントリ間の重複拍数。
                1 の場合、次のエントリが前のエントリの最終拍と同時に開始
                （BWV 1080のように主題最終音と次エントリ冒頭音が同時発音）。
                0 の場合、従来通り重複なし。
        """
        entries = []
        ...
        for i, voice_type in enumerate(voice_order):
            ...
            # overlap > 0 の場合、主題最終拍と次エントリ冒頭拍が重なるため
            # コデッタは不要（BWV 1080 等のスタイル）
            if i < len(voice_order) - 1:
                if overlap > 0:
                    # overlap方式: コデッタ不要
                    position += self.subject.get_length() - overlap
                else:
    ...
    def create_episode(self, start_position: int,
                       ...
                       step_interval: int = -1,
                       end_key: Optional[Key] = None) -> Episode:
        ...
        Args:
            ...
            motif_length: 動機の長さ
            sequence_steps: 反復回数
            step_interval: 移調幅（音階度数、負=下行。-1=2度下行）
            end_key: 終点調（Noneなら主調のまま）
        """
        ...
        episode = Episode(
            ...
            source="主題",
            key=self.main_key,
            start_key=self.main_key,
            end_key=end_key if end_key is not None else self.main_key,
        )
